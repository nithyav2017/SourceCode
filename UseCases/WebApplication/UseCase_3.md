...
#### Root Cause: 
             ...
             As a result,  if a user's subsequent request is handled by a different server than the one that originally created the session, 
             the new server cannot find the session data in its memory and leading to session loss. For example, user appears logged out .
